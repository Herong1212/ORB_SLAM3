...
namespace ORB_SLAM3
{
    ...
    public:
        // todo 作用：结构体，默认构造函数
        ORBmatcher(float nnratio = 0.6,   // 最优和次优评分的比例
                   bool checkOri = true); // 是否检查方向
        ...
        // ps：Used in loop detection (Loop Closing)
        // Project MapPoints using a Similarity Transformation and search matches.
        /**
         * @brief 根据 Sim3 变换，将每个 vpPoints 投影到 pKF 上，并根据尺度确定一个搜索区域，
        ...
        // ps：Used in Place Recognition (Loop Closing and Merging)
        // Project MapPoints using a Similarity Transformation and search matches.
        ...
        // 通过词袋搜索匹配，用于局部建图线程中两两关键帧之间尚未匹配特征点的快速匹配，为了生成新的匹配点对
        /**
        ...
        int Fuse(KeyFrame *pKF,                         // 关键帧
                 const vector<MapPoint *> &vpMapPoints, // 地图点
                 const float th = 3.0,                  // 搜索窗口的阈值
                 const bool bRight = false);

        // 将地图点投影到关键帧中进行,但是由于种种原因,地图点还不能够在这个函数中完成替换操作
        // Project MapPoints into KeyFrame using a given Sim3 and search for duplicated MapPoints.
        int Fuse(KeyFrame *pKF,                           // 关键帧
                 Sophus::Sim3f &Scw,                      // 仿射变换
                 const std::vector<MapPoint *> &vpPoints, // 给出的地图点
                 float th,                                // 搜索窗口阈值
                 vector<MapPoint *> &vpReplacePoint);     // 需要替换掉的地图点,键值对
    ...
    protected:
         ...
         * @return false
         */
        bool CheckDistEpipolarLine(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &F12, const KeyFrame *pKF); // ! 在 2 中用到过这个，在 3 中好像没有用到。
        ...
        /**
         * @brief 筛选出在旋转角度差落在在直方图区间内数量最多的前三个 bin 的索引
         *
